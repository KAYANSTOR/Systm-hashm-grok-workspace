...
test("parseWriteAtomicArgs needs exactly a staged file and a target", () => {
  assert.deepEqual(parseWriteAtomicArgs([".app/og.tmp", "public/og.jpg"]), {
    staged: ".app/og.tmp",
    target: "public/og.jpg",
  });
  assert.match(parseWriteAtomicArgs([]).error, /usage:/);
  assert.match(parseWriteAtomicArgs([".app/og.tmp"]).error, /usage:/);
...
test("stagingError refuses a temp inside public/ and a no-op move", () => {
  ...
  assert.equal(
    stagingError({
      staged: "/workspace/.app/og.jpg.tmp",
...
test("handOver replaces the target and clears the staged file", () => {
  ...
  const staged = join(root, ".app/og.jpg.tmp");
...
test("handOver creates a missing target directory", () => {
  const root = makeWorkspace();
  const staged = join(root, ".app/site.json.tmp");
...
test("an interrupted pass leaves the target on its old bytes, temp-free", () => {
  ...
  writeFileSync(join(root, ".app/og.jpg.tmp"), "half a JPEG");

  assert.equal(readFileSync(target, "utf8"), "old card");
  assert.throws(() => handOver(join(root, ".app/absent.tmp"), target), { code: "ENOENT" });
  assert.equal(readFileSync(target, "utf8"), "old card");
...
test("a failed hand-over creates no directory for the target it never wrote", () => {
  const root = makeWorkspace();
  const missing = join(root, ".app/absent.tmp");
...
test("cli: hands the file over, and refuses a temp staged in public/", () => {
  const root = makeWorkspace();
  writeFileSync(join(root, ".app/og.jpg.tmp"), "new card");
  const ok = spawnSync(
    process.execPath,
    [SCRIPT, join(root, ".app/og.jpg.tmp"), join(root, "public/og.jpg")],
    { encoding: "utf8" },
  );
...
test("every hand-over the og skill prints is one this script accepts", () => {
  ...
  const skillDir = join(TEMPLATE_ROOT, ".app/skills/og");
  const docs = [
...
test("cli: a missing staged file fails without touching the target", () => {
  ...
  const run = spawnSync(
    process.execPath,
    [SCRIPT, join(root, ".app/absent.tmp"), join(root, "public/og.jpg")],
    { encoding: "utf8" },
  );
