 ...
 *
 *   node scripts/write-atomic.mjs /workspace/.app/og.jpg.tmp public/og.jpg
 *
 ...
 * /workspace/.app/" contract rests on — cannot be provoked portably.
 */
    ...
    if (err?.code === "EXDEV") {
      throw new Error(
        `${staged} is on another filesystem than ${target}, so the hand-over cannot be a `
          + "rename — stage under /workspace/.app/ instead",
      );
    }
    throw err;
  }
