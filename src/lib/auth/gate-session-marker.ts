/**
 * Client-readable marker for gate-materialized sessions ("Sign in with App"
 ...
 * `x-app-identity` — so `UserButton` uses this to hide its sign-out control.
 * `__Host-` prefixed like the other auth cookies: browsers reject a `__Host-`
 * cookie carrying a `Domain`, so an untrusted sibling `*.app.me` app cannot
