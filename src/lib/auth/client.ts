 ...
 * (`.app/app-env.json`), which selects the dev user (see `use-current-user`);
...
/**
 * The sandbox live preview runs this app inside an iframe on a `*.app-sandbox.com`
...
/**
 * Start sign-in with one upstream provider (`providerId` from `APP_PROVIDERS`),
 * federating through the App auth broker.
 *
 * - **Live preview** (`*.app-sandbox.com` iframe): opens a POPUP to
