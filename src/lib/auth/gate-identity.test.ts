...
describe("gateIdentityFromHeaders", () => {
  it("verifies the header token end to end and fails closed without it", async () => {
    ...
    try {
      ...
      const withToken = await gateIdentityFromHeaders(
        new Headers({ "x-app-identity": token }),
        fetchImpl,
      );
  ...
  it("activates on a deployed-shaped request without APP_GATE_ORIGIN", async () => {
  ...
  it("verifies a preview-audience token with no gate env vars via the loopback default", async () => {
    ...
    const identity = await gateIdentityFromHeaders(
      new Headers({
        host: "my-session.app-sandbox.com",
        "x-app-identity": token,
      }),
      fetchImpl,
    );
  ...
  it("verifies a preview-audience token when only APP_GATE_ORIGIN is set", async () => {
    const key = await makeKey("k1");
    const { fetchImpl } = staticJwks([key.jwk]);
    delete process.env.APP_PROJECT_ID;
    process.env.APP_GATE_ORIGIN = ISSUER;
  ...
  it("rejects a preview-audience token when APP_PROJECT_ID is set", async () => {
    const key = await makeKey("k1");
    const { fetchImpl } = staticJwks([key.jwk]);
    process.env.APP_PROJECT_ID = "proj-123";
    process.env.APP_GATE_ORIGIN = ISSUER;
...
describe("gateTokenAudience", () => {
  it("pins app:<id> when APP_PROJECT_ID is set, even alongside APP_GATE_ORIGIN", () => {
    process.env.APP_PROJECT_ID = "proj-123";
    process.env.APP_GATE_ORIGIN = ISSUER;
    try {
  ...
  it("pins preview when APP_PROJECT_ID is unset", () => {
    delete process.env.APP_PROJECT_ID;
    process.env.APP_GATE_ORIGIN = ISSUER;
    try {
