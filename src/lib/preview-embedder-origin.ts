export function isAppEmbedderOrigin(origin: string): boolean {
  try {
    const url = new URL(origin);
    if (url.protocol !== "https:" && url.protocol !== "http:") return false;
    const host = url.hostname.toLowerCase();
    if (host === "app.com" || host.endsWith(".app.com")) return true;
...
export function isSandboxPreviewGuestHost(hostname: string): boolean {
  ...
  return host === "app-sandbox.com" || host.endsWith(".app-sandbox.com");
}
...
export function resolveParentEmbedderOrigin(
  ...
  for (const candidate of [referrer, ancestorOrigin ?? ""].filter(Boolean)) {
    try {
      ...
      if (isAppEmbedderOrigin(url.origin)) return url.origin;
      if (
