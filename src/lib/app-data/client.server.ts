...
function connectorsBaseFor(publicHost: string | null): string | null {
  const explicit = env("APP_CONNECTORS_URL");
  ...
  if (host === "app.me" || host.endsWith(".app.me")) {
...
function inboundContext(): InboundContext {
  ...
  const envToken =
    process.env.NODE_ENV === "production"
      ? null
      : (env("APP_CONNECTOR_ACCESS_TOKEN") ?? null);
  return {
...
function tokenIdentityKey(token: string): string {
  ...
  if (payload) {
    try {
      ...
      // Ignore invalid JWT, fallback to hashing the token directly.
    }
  }
