...
export default function EmployeesPage() {
  ...
  async function onCreateEmployee(e: FormEvent) {
    ...
    try {
      ...
      await createEmployeeAccount({
        data: {
          employeeId: String((created as any).id),
  ...
  async function onResetPassword(employeeId: string) {
    ...
    try {
      ...
      setError("");
      window.alert("تم تغيير كلمة المرور. استخدم الرقم وكلمة المرور الجديدة للدخول.");
    } catch (e) {
  ...
  async function onCreateAccount(employeeId: string) {
    ...
    try {
      ...
      setError("");
      window.alert("تم إنشاء حساب الدخول. يسجّل الموظف برقم هاتفه وكلمة المرور.");
      await load();
    } catch (e) {
    ...
    <main dir="rtl" className="min-h-screen bg-stone-100 p-6 text-zinc-900">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              بيانات الموظف: الاسم ورقم الهاتف. كلمة المرور تستخدم لحساب الدخول ولا تُخزن في ملف
              الموظف.
            </p>
          </div>
          <Link
        ...
        >
          ...
          >
            {saving ? "جارٍ إنشاء الحساب…" : "إضافة الموظف"}
          </button>
        </form>
                  ...
                  {employees.map((e) => (
                    <tr key={e.id} className={!e.is_active ? "opacity-50" : ""}>
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-zinc-500">{e.phone || "—"}</div>
                      </td>
                      <td className="px-4 py-3">{e.user_id ? "مرتبط بحساب دخول" : "غير مرتبط"}</td>
                      <td className="px-4 py-3">
                        {e.user_id ? (
                          <button
                            type="button"
