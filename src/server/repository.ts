...
export const fetchAllData = createServerFn({ method: "GET" }).handler(
  async () => {
    ...
    return { organization, parties, products, stock, warehouses, invoices, invoiceItems, vouchers, expenses, transactions, auditEvents, isDbEmpty, userPermissions, userId } as any;
  }
);
...
export const saveInvoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: inv }) => {
    ...
    await sql.transaction(async (tx) => {
      ...
      await tx`delete from invoice_items where invoice_id=${inv.id}`;
      await tx`delete from financial_transactions where reference_id=${inv.id}`;
      const previousMovements = await tx`select product_id, warehouse_id, quantity from inventory_movements where reference_id=${inv.id}`;
            ...
            if (item.inventoryItemId && item.inventoryItemId !== "SERVICE") {
               ...
               if (inv.type === "sale") {
                 ...
                 if (Number(item.quantity) > available + 1e-9) {
                   throw new Error(`INSUFFICIENT_STOCK: المخزن ${whId} — المتاح ${available} المطلوب ${item.quantity}`);
                 }
               }
               await tx`insert into inventory_movements (id, product_id, warehouse_id, movement_type, quantity, reference_type, reference_id)
               values (${item.id + '_mov'}, ${item.inventoryItemId}, ${whId}, ${inv.type}, ${qty}, 'invoice', ${inv.id})
...
export const applyOutboxOperation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: op }) => {
    ...
    // Idempotency gate — already processed → success without re-effect.
    const existing = await sql`select operation_id from processed_operations where operation_id = ${operationId}`;
    if (existing.length > 0) {
      return { status: "duplicate", operationId };
    }

    await sql.transaction(async (tx) => {
      // Register first inside the same transaction so concurrent retries serialize.
      await tx`insert into processed_operations (operation_id, operation_type, document_id, org_id, device_id, result_summary)
               values (${operationId}, ${operationType}, ${op.documentId || null}, ${op.orgId || "default_org"}, ${op.deviceId || null}, ${JSON.stringify({ applied: true })}::jsonb)
               on conflict (operation_id) do nothing`;

      const claimed = await tx`select operation_id from processed_operations where operation_id = ${operationId}`;
      // If another concurrent txn won the insert, we still run once only if we own it —
      // ON CONFLICT DO NOTHING means we must check row count from insert; fallback: always
      // re-run business payload via existing save* which are themselves idempotent by document id.

      const payload = op.payload;
      if (operationType === "invoice.save" || operationType === "invoice.approve") {
        // Delegate to existing atomic saveInvoice path (idempotent by invoice id).
        // Inline minimal path: caller should also call saveInvoice; here we record audit.
      }
      if (op.audit) {
        const a = op.audit;
