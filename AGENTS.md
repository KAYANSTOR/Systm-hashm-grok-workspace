# ⚠️ توجيه إلزامي لكل وكيل ذكاء اصطناعي يعمل على هذا المستودع

قبل أي سطر كود، اقرأ هذين الملفين في جذر المشروع — وهما **مصدر الحقيقة الوحيد** لحالة المشروع وخطة تطويره، ويُقدَّمان على أي تعليمات عامة أدناه:

1. **`PROJECT_STATUS.md`** — الحالة الفعلية الدقيقة الآن: ما أُنجز فعليًا (مُتحقَّق منه بالكود)، وما لا يزال مفتوحًا.
2. **`ROADMAP.md`** — خطة التطوير المرحلية **الإلزامية** من هذه النقطة حتى الإنتاج.

**قواعد ملزمة على أي وكيل (Claude، Codex، أو غيرهما):**

- **الالتزام الصارم بترتيب المراحل** في `ROADMAP.md`. لا تُنفَّذ مرحلة جديدة قبل إنجاز معيار الإنجاز الخاص بالمرحلة التي تسبقها، ولا يُعمل خارج مسار الخطة دون طلب صريح ومباشر من صاحب المشروع في نفس المحادثة.
- **قبل أي `commit`**: نفّذ `git fetch` وراجع `git log origin/main` — قد يكون وكيل آخر يعمل بالتوازي على نفس المستودع. ادمج (`rebase`/merge) بعناية بدل الكتابة فوق عمل غيرك، وتحقّق من عدم التكرار أو التعارض معه (خصوصًا في `src/lib/auth/**`).
- **بعد أي عمل حقيقي**: أضف قسمًا مؤرَّخًا في `PROJECT_STATUS.md` يوثّق ما فعلته والسبب، حتى لا يفقد أي وكيل تالٍ — بشريًا كان أو آليًا — السياق.
- **قبل أي `push`**: شغّل `npm run typecheck && npm test && npm run lint && npx vite build` وتأكد من نجاحها كلها. لا يُدفع كود لم يُتحقَّق منه.
- **ممنوع** تعديل خطة `ROADMAP.md` جوهريًا (حذف مرحلة، تغيير أولويتها) إلا لتحديثها بواقع جديد يعكس تقدّمًا فعليًا — وليس لتغيير القرار الاستراتيجي نفسه دون طلب صريح من صاحب المشروع.

القسم الطويل أدناه هو سقالة "Grok App Builder" الأصلية التي أُنشئ بها المشروع مبدئيًا. النشر الفعلي الآن يتم مباشرة عبر Vercel، لا عبر هذه المنصة، لذا لا يُعتد بالتعليمات أدناه إن تعارضت مع `ROADMAP.md` أو `PROJECT_STATUS.md` أو مع طلب صريح من صاحب المشروع.
...
App Build, in an isolated Linux sandbox; read it fully before writing code.
...
"App Dashboard instead": it serves viewer connector data via the gate;
...
- **Never delete or overwrite `public/__app/`, `server/`, `scripts/app-pwa-*`**
...
- **`npm install` works** for JS packages; game engines (`three`, Phaser) are
  **not** preinstalled, so install them and leave them in `package.json` for
  ...
  needs `APP_ALLOW_INSTALL_SCRIPTS=1 npm install <pkg>`.
...
- **`AI_API_KEY` in the env** = real, server-only AI access spending the **app
  owner's quota**: read **`ai-api`** first, keep calls user-initiated and
...
2. **Keep the branding injector** — `appPwaPlugin()` and
   `server/middleware/app-pwa.ts` inject
...
5. **Auth routes only when §0.5 says accounts** — then add `src/routes/login.tsx`
   ...
   inside the popup. Viewers opened from App are gate-signed-in with zero
   clicks — **never render "Sign in / Re-auth with App" buttons** outside the
   `app-data` skill's `login` error state. Wiring:
   `.app/references/data-and-auth.md`.
...
6. **Brand-asset pass — a subagent, never waited for.** Custom-card app per
   ...
   the placeholder card. Meanwhile it keeps `/workspace/.app/og-pending` fresh
...
7. **Verify it actually RENDERS — mandatory, before you say it's done.** A 200
   ...
   **`agent-browser`** CLI, not a hand-written Playwright script; read
   `.app/references/browser-qa.md` first.
